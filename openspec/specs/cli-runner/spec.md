...
Dev-Phase 0 的唯一人机交互入口。`cli-runner` 通过 `sddp run` 命令启动 engine-core Flow，承担三件事：(1) 解析自然语言 proposal 字符串或文件路径作为输入；(2) 在 3 个用户确认点（需求 / 方案 / 任务）通过 stdin/stdout 阻塞并与 CrewAI @human_feedback 适配；(3) 支持 @persist 中断后从本地 SQLite 恢复流程，并将 engine-core 产出的文档与 `cost_report.json` 写入 `--output` 目录。对应 `dod.md` D0-4、D0-9。
