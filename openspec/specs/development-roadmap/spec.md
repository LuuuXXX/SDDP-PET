...
为 SDDP-PET 项目定义显式、可寻址、可度量的开发路线图：将系统分解为边界清晰的模块，按 Dev-Phase 顺序与依赖推进，每个阶段具备二元判定的 DoD、Go/No-Go 门槛与回退路径，并以独立的 opsx-apply 变更逐阶段实现与验收。所有技术决策可追溯到 `analysis/` 中已有的分析文档。
