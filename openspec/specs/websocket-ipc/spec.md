...
Dev-Phase 1 的前后端双向通信协议层。`websocket-ipc` 在后端引擎暴露 FastAPI WebSocket server（端口 8765），定义引擎 → 前端的 5 种 Push 消息（`agent_state_change` / `document_produced` / `cost_update` / `feedback_required` / `error`）与前端 → 引擎的 4 种 RPC 请求（`start_flow` / `user_feedback` / `resume_flow` / `abort_flow`，通过 `message_id` UUID 关联请求响应）；心跳使用应用层 JSON `{"type":"ping"}`（非 RFC 6455 协议帧），连续 3 次未收到 pong 触发连接丢失事件；`WebSocketHumanFeedbackAdapter` 桥接 WS 协议到 DP0 `LinearPhase02Flow.human_feedback_handler` 抽象，替换 CLI 模式下的 `CLIHumanFeedbackAdapter`。对应 `dod.md` D1-4、D1-5、D1-6、D1-7。权威来源：`analysis/00` §七、`analysis/08`。
