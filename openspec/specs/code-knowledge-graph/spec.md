...
Dev-Phase 0 交付的 KG-MVP（最小可行版）：以 SCIP 索引器（Python 起步）+ tree-sitter fallback 扫描本地代码库，产出 SQLite 知识图（5 类节点 / 8 类边）；对外提供 `KnowledgeGraphQueryAPI`（4 类带置信度的查询）；内置准确性验证套件，把召回率映射到 `Confidence` 枚举，并把召回率 ≥ 0.70 作为 Dev-Phase 0 的 Go 门槛（`dod.md` D0-6）。范围限定为 Python 单语言、不含增量更新/多语言/远程模式（推迟到 KG-v1）。权威来源：`analysis/02-code-knowledge-graph-design.md`。
