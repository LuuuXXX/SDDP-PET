...
Dev-Phase 0 的核心编排层：经 `adaptation-layer` 驱动一个 CrewAI Flow，按线性顺序流转 5 个角色（需求官 / 调度官 / 架构师 / 实施师 / 代码资产管理员），将自然语言 proposal 转换为结构化的 proposal / delta-spec / delta-design 三种文档。强制使用 `output_pydantic` 做输出 schema 校验、提供 JSON ↔ Markdown 双向渲染、内置 token 计量产出 `cost_report.json`；并把实施师限定为仅产出代码建议、不自动写文件。对应 `dod.md` D0-7、D0-8、D0-9、D0-11/12/13。权威来源：`analysis/00-sddp-pet-final.md`。
